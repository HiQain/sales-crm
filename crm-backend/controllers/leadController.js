...
export const createLead = async (req, res) => {
  const {
    contact, email, business_name, service, response,
    follow_up, lead_value, lead_status, assigned_user
  } = req.body;

  try {
    const [result] = await db.execute(`
      INSERT INTO leads 
      (contact, email, business_name, service, response, follow_up, 
       lead_value, lead_status, assigned_user, created_by)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [
      contact || 'New Lead',
      email,
