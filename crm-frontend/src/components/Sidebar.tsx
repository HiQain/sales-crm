    ...
    >
        ...
        {links.map((link) => (
          <NavLink
            ...
            className={({ isActive }) => 
              ...
              } ${isCollapsed ? 'mx-auto h-14 w-14 justify-center px-0 py-0' : 'gap-3 px-4 py-3'}`
            }
            title={isCollapsed ? link.label : undefined} // Tooltip when collapsed
          >
            <link.icon size={20} className="flex-shrink-0" />
            <span className={`font-medium transition-all duration-200 ${isCollapsed ? 'hidden' : 'opacity-100'}`}>
              {link.label}
            </span>
          </NavLink>
        ))}
      ...
      <div className="mt-auto p-4 border-t border-white/20 bg-white/20">
        ...
        <button 
          ...
          className={`cursor-pointer w-full flex items-center justify-center gap-2 py-3 text-slate-500 hover:text-rose-600 hover:bg-rose-500/10 rounded-xl transition-all font-bold text-sm ${
